...
import { WithAnchorV0210 } from "../target/types/with_anchor_v0_21_0";
...
describe("cpi_whirlpool_anchor_gen_v0201", () => {
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.Provider.env());

  const connection = anchor.getProvider().connection;
  const program = anchor.workspace.WithAnchorV0210 as Program<WithAnchorV0210>;
  const provider = anchor.getProvider();
  const wallet = anchor.Provider.env().wallet;
  ...
  it("load whirlpools config account", async () => {
    ...
    const signature = await program.rpc.verifyWhirlpoolsConfigAccount({
      accounts: {
  ...
  it("load fee tier 1 account", async () => {
    ...
    const signature = await program.rpc.verifyFeetierAccount({
      accounts: {
  ...
  it("load whirlpool account", async () => {
    ...
    const signature = await program.rpc.verifyWhirlpoolAccount({
      accounts: {
  ...
  it("load tickarray account", async () => {
    ...
    const signature = await program.rpc.verifyTickarrayAccount(
  ...
  it("load SOL/USDC position account", async () => {
    ...
    const signature = await program.rpc.verifyPositionAccount({
      accounts: {
  ...
  it("execute proxy swap SOL to USDC", async () => {
    ...
    const swap = await program.instruction
      .proxySwap(
        amount,
    ...
    const pre_usdc = pre_usdc_ta === null ? new anchor.BN(0) : pre_usdc_ta.amount as anchor.BN;
    ...
    const post_usdc = post_usdc_ta.amount as anchor.BN;
  ...
  it("execute proxy swap USDC to SAMO", async () => {
    ...
    const pre_samo = pre_samo_ta === null ? new anchor.BN(0) : pre_samo_ta.amount as anchor.BN;

    const signature = await transaction.buildAndExecute();
    await connection.confirmTransaction(signature);

    const post_samo_ta = await fetcher.getTokenInfo(samo_ta.address, true);
    const post_samo = post_samo_ta.amount as anchor.BN;
  ...
  it("execute proxy open_position", async () => {
    ...
    const open_position = await program.instruction
      .proxyOpenPosition(
        bumps,
        tick_lower_index,
        tick_upper_index, {
  ...
  it("execute proxy increase_liquidity", async () => {
    ...
    const increase_liquidity = await program.instruction
  ...
  it("execute proxy update_fees_and_rewards", async () => {
    ...
    const update_fees_and_rewards = await program.instruction
  ...
  it("execute proxy decrease_liquidity", async () => {
    ...
    const decrease_liquidity = await program.instruction
  ...
  it("execute proxy collect_fees", async () => {
    ...
    const collect_fees = await program.instruction
      .proxyCollectFees({
        accounts: {
          whirlpoolProgram: ORCA_WHIRLPOOL_PROGRAM_ID,
          whirlpool: samo_usdc_whirlpool_pubkey,
  ...
  it("execute proxy collect_reward", async () => {
    ...
    for (let reward_index=0; reward_index<3; reward_index++) {
      ...
      const collect_reward = await program.instruction
        .proxyCollectReward(
          reward_index, {
          accounts: {
            whirlpoolProgram: ORCA_WHIRLPOOL_PROGRAM_ID,
  ...
  it("execute proxy close_position", async () => {
    ...
    const close_position = await program.instruction
      .proxyClosePosition({
        accounts: {
          whirlpoolProgram: ORCA_WHIRLPOOL_PROGRAM_ID,
          positionAuthority: wallet.publicKey,
